    ...
    (async () => {
      try {
        const res = await api<{ table: { number: number }; session: import("@/stores/table").TableSession }>(
          "/api/tables/scan",
          { method: "POST", body: JSON.stringify({ qr_token: t }) }
        );
        setSession(res.session);
        toast.success(`Welcome to Table ${res.table.number} — held for 10 minutes`);
      } catch (e) {
        const err = e as Error;
        toast.error(err.message || "Invalid table QR");
      } finally {
        // Strip the t= param from the URL
